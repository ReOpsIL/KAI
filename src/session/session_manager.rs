...
impl SessionManager {
    ...
    pub fn delete_session(&mut self, identifier: &str) -> SessionResult {
        ...
        let session_id = if let Some(_session) = self.sessions.get(identifier) {
    ...
    pub fn list_sessions(&self, name_filter: Option<&str>) -> SessionResult {
        ...
        let session_list = sessions
            .iter()
            .map(|s| {
                let active = if self.active_session_id.as_ref() == Some(&s.id) {
                    " (active)"
                } else {
                    ""
                };
