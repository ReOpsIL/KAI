...
use crate::planer::plan::{Plan, Phase};
use crate::planer::queue::{ExecutionQueue, QueueRequest, QueueResponse};
use crate::planer::schemas::{PlanResponse, PlanPhase, SimpleTask};
...
impl TaskPlanner {
    ...
    pub async fn create_advanced_plan(&mut self, user_input: &str) -> Result<String, String> {
        ...
        // Generate LLM prompt using the prompt manager
        let system_prompt = PromptManager::get_system_prompt();
        ...
        let plan_response: PlanResponse = serde_json::from_str(&content)
            .map_err(|e| format!("Failed to parse LLM response as JSON: {}. Response: {}", e, content))?;
        ...
        Ok(format!("{}\n\n✅ Plan created successfully with {} tasks queued for execution.", plan_display, request_ids.len()))
    }


    /// Convert LLM PlanResponse to internal Plan structure
    fn convert_plan_response_to_plan(&mut self, plan_response: PlanResponse) -> Result<Plan, String> {
    ...
    pub fn process_next_request(&mut self) -> Option<QueueResponse> {
        ...
        let response = match &request {
            QueueRequest::UserPrompt { content, .. } => {
                self.handle_user_prompt(content)
            }
            QueueRequest::TaskExecution { task, .. } => {
                self.handle_task_execution(task)
            }
        };
    ...
    pub fn find_plan_by_title(&mut self, title: &str) -> Option<&mut Plan> {
        self.active_plans.iter_mut().find(|p| p.title.contains(title))
    }
    ...
    fn handle_user_prompt(&mut self, content: &str) -> QueueResponse {
        ...
        QueueResponse {
            request_id,
            success: false,
            content: format!("User prompt '{}' requires AI planning - no dummy processing available", content),
