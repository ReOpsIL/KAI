...
impl Harvester {
    ...
    fn should_include_file(&self, path: &Path) -> bool {
        if let Some(extension) = path.extension() {
            if let Some(ext_str) = extension.to_str() {
                return self.config.include_extensions.contains(ext_str.to_lowercase().as_str());
            }
        }
    ...
    async fn generate_file_description(&self, file_info: &FileInfo) -> Result<String, Box<dyn std::error::Error>> {
...
{}"#,
    ...
    pub async fn generate_file_descriptions(&self, files: &mut [FileInfo]) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for file_info in files.iter_mut() {
            match self.generate_file_description(file_info).await {
                Ok(description) => {
                    ...
                    println!("Generated description for: {}", file_info.relative_path.display());
                }
                Err(e) => {
                    eprintln!("Failed to generate description for {}: {}", file_info.relative_path.display(), e);
    ...
    pub async fn generate_module_descriptions(&self, modules: &mut [ModuleInfo]) -> Result<(), Box<dyn std::error::Error>> {
        let client = self.openrouter_client.as_ref()
            .ok_or("OpenRouter client not configured")?;
        
        for module in modules.iter_mut() {
            let file_summaries: Vec<String> = module.files.iter()
                .filter_map(|f| {
...
{}"#,
            ...
            match client.send_prompt(
                ...
                Err(e) => {
                    eprintln!("Failed to generate module description for {}: {}", module.name, e);
                }
            }
...
mod tests {
    ...
    async fn test_file_discovery() {
        ...
        assert!(files.iter().any(|f| f.relative_path == Path::new("src/main.rs")));
        assert!(files.iter().any(|f| f.relative_path == Path::new("Cargo.toml")));
    }
}